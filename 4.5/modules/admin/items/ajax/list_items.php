...
if(!$item_id){
	...
	if($_POST['post_copy']){
		...
		while(list($n,$v)=each($_POST['sel'])){
			if($_POST['removeall']) {
                $emps->db->query("delete from " . TP . "ws_items_structure where item_id=" . intval($n));
            }elseif($_POST['removesel']){
			    foreach($_POST['othernode'] as $other_node){
                    $emps->db->query("delete from ".TP."ws_items_structure where item_id=".intval($n)." and structure_id = ".intval($other_node));
                }

			}else{
				reset($_POST['othernode']);
				while(list($nn,$nv)=each($_POST['othernode'])){
		...
		if($node_id==-1){
			...
			$q = "select SQL_CALC_FOUND_ROWS i.*, (match(i.name,p.v_char,p.v_text,p.v_data) against ('$rtxt' in boolean mode)) as rel,(i.name like ('$ptxt')) as namel,(i.article like ('$ptxt')) as articlel, 
			 ...
			 on (ctx.ref_id  = i.id and ctx.ref_type = ".DT_WS_ITEM.")
...
//		dump($lst);
	...
	$photos->photo_size="1600x1600|100x100|inner";
	...
	if($_POST['post_save']){
		if($_POST['pic_take']){
			while(list($n,$v)=each($_POST['pic_take'])){
